        ...
        public static void StreamListCheck(List<LiveStreamer> list, int StreamCheckDelay)
        {
            try
            {
                foreach (var item in list)
                {
                    ...
                    {
                        Program.Client.Logger.LogDebug(CustomLogEvents.LiveStream, "User {UserName} removed from Live Stream List - {CheckDelay} hours passed.", item.User.Username, LiveStream.StreamCheckDelay);
                        list.Remove(item);
                    }
                }
            }
